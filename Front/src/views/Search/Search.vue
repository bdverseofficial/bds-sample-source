      ...
      <v-col cols="3">
        ...
        <template v-if="result && result.facets && result.items">
          ...
          <v-list
            dense
            shaped
            v-for="(item, indexFacet) in result.facets.items"
            :key="indexFacet"
          >
            <v-list-group :value="item.selected">
              <template v-slot:activator>
                <v-list-item-title
                  v-text="item.facet.localName || item.facet.name"
                ></v-list-item-title>
                <v-list-item-icon
                  v-if="item.selected"
                  @click="search(item.uri)"
              ...
              >
                <v-list-item-title
                  v-text="value.facetValue.localName || value.facetValue.name"
                ></v-list-item-title>
      ...
      <v-col cols="9">
        ...
        <v-list three-line v-if="result" class="data-table-view">
          <template v-for="(item, index) in result.items">
            <v-list-item :key="'I' + index">
              <v-list-item-content>
                ...
                <v-list-item-subtitle>
                  <span
                    >{{ $t("SEARCH.GROUPSIZE") }}:{{ item.item.groupSize }}
                  </span>
                  <span
                    >{{ $t("SEARCH.GROUPTYPE") }}:{{ item.item.groupType }}
                  </span>
                </v-list-item-subtitle>
              </v-list-item-content>
              <v-list-item-action v-if="profileStore.me">
                <template
                  v-if="
                    profileStore.me.preferredSport &&
                    profileStore.me.preferredSport.key == item.item.key
                  "
                >
                  <v-btn icon><v-icon>star</v-icon></v-btn>
                </template>
                <template v-else>
                  <v-btn icon @click="updateSport(item.item)"
            ...
            <v-divider
              v-if="index + 1 < result.items.length"
              :key="index"
            ></v-divider>
          </template>
        </v-list>
